...
This installs Python deps and pulls two models (`qwen3:8b`, `gemma3:4b`). Edit the script if you want different models.

### 2. Run the baseline

```bash
python bench.py --model qwen3:8b --out results/qwen3-8b.json
python bench.py --model gemma3:4b --out results/gemma3-4b.json
```
...
```
Task              qwen3:8b   gemma3:4b
coding/fizzbuzz   3/3 (1.2s) 2/3 (0.8s)
coding/palindrome 3/3 (0.9s) 3/3 (0.6s)
tool_use/weather  3/3 (1.1s) 1/3 (0.7s)
extraction/person 3/3 (0.8s) 3/3 (0.5s)
overall           100%       75%
```
...
Or against Anthropic via an OpenAI-compatible proxy. The point is to see how close the local models get.

## Suggested challenges

Pick one and ship by demo time:

1. **The Coding Gauntlet** — Replace `tasks/coding.py` with 50 real GitHub issues. Score by whether the patch passes the original test suite. Bonus: graph quality vs. context length.
...
4. **Refusal & Drift Audit** — Add `tasks/refusal.py` with 100 benign-but-touchy prompts. Measure refusal rate per model.
5. **Quantization Sweep** — Pull `qwen3:8b-q4_K_M`, `qwen3:8b-q8_0`, and `qwen3:8b-f16`. Show the quality cliff (if any).
